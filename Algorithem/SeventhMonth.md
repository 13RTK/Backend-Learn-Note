...
![Xnip2021-10-16_10-51-53](Algorithem/Xnip2021-10-16_10-51-53.jpg)



![Xnip2021-10-16_11-17-01](Algorithem/Xnip2021-10-16_11-17-01.jpg)

题意:

给你一个数组，请你计算中其中最大的严格递增的子数组之和，并返回构成该最大和对应的数字
...
- 获取子数组和其实很简单，就是一个简单的动态规划罢了，我们创建一个与源数组长度一致的数组，其每个元素的值代表前面所有范围内的最大递增子数组和
- 其获取其中每个元素的formula如下:
- 遍历原数组array，索引为i
- 每次循环中都从0到i遍历原数组，索引设为j
- 判断array[j] < array[i]和sums[i] < sums[j] + array[i]两个条件是否成立
- 成立则sums[i] = sums[j] + array[i]
- 且每次更新后我们都判断该位置的数是否为最大和值，最终获取该sums数组中最大和值所在的索引值即可
- 至此，我们获取了最大和值的索引，可以通过sums数组获取递增子数组的最大和值了，但还无法得知构成这个最大和值的元素是哪些



- 为了解决该问题，我们需要创建一个sequence数组，长度仍然同原数组相同，但需要给定一个初始值
- 其中每个索引位置上的值，都对应使得sums数组同位置上的数被更新的元素索引，有些费解，这里举个栗子:

对于原输入数组:

{8, 12, 2, 3, 15, 5, 7}

创建一个sums数组后，该数组最终会更新为:

{8 20 2 5 35 10 17}

对应的，sequence数组对应值为(初始值暂时用-1替代，实际测试用例中会有负数，-1并不严谨):

-1 0 -1 2 1 3 5



- 更新完sums数组后，很明显其最大递增子数组的和值为35，对应索引值为4
- 通过这个索引在sequence数组中反向逆推即可获取所有的构成元素
- 那么sequence数组的形成过程是怎么的呢?
- 其中索引为1的数明显被更新为了0，其意味着在更新sums中索引为1的值时，使得该数字更新的元素在原数组中的索引为0(即数字8)
- 因为当i = 1, j = 0时，array[0] = 8 < array[1] = 12且 8 + 12 > sums[i]，此时sums[i]值被更新
- 而使其更新的值为8，其对应索引值为0，所以我们在索引为i的位置记录下使sums[i]被更新的值在原数组中对应的索引值j
- 从而得到了数组sequence，之后我们反向遍历该sequence数组即可获取所有构成最大递增子数组的数字
...
- 我们遍历了一次原数组，但每次循环中都有一个内循环，所以时间复杂度为O(n^2)
- 我们创建了两个长度为n的数组，所以空间复杂度为O(n)
...
## Tag: Dynamic Programming, Array, String, Longest common subsequence

![Xnip2021-10-17_15-35-58](Algorithem/Xnip2021-10-17_15-35-58.jpg)



![Xnip2021-10-17_15-36-20](Algorithem/Xnip2021-10-17_15-36-20.jpg)



![Xnip2021-10-17_15-35-26](Algorithem/Xnip2021-10-17_15-35-26.jpg)



![Xnip2021-10-17_15-38-42](Algorithem/Xnip2021-10-17_15-38-42.jpg)

题意:

给你两个字符串，请你找出它们其中最长的公共子序列
...
- 很明显又是一道动态规划的题目，我们同样可以创建一个二维数组，每个元素都代表当前索引范围内的最长子序列，最后返回二维数组右下角的值即可
- 这样做确实清晰明了，但问题是我们的时空复杂度都从O(n * m)变为了O(n * m * min(n, m))，因为每个元素存放的不是单个字符而是字符串
- 为了减少复杂度，我们选择创建一个二维int类型的数组，每个元素都只存放当前索引范围内两字符串的最长公共子序列的长度
- 同样的给出每个元素的formula(索引从1开始):
- 如果当前索引对应两字符串中的字符相同，则将该位置的元素设置为其对角线上元素的值 + 1(意为最长子序列的长度 + 1)
- 不同则设置为其左边或上面位置中最大的元素值(意为长度不变)
- 遍历完这个循环后，该二维数组的右下角元素值就是最长公共子序列的长度了，但我们不要长度，要具体的字符呀



- 其实通过这个二维数组我们可以获取答案:
- 从二维数组的右下角元素开始，将当前位置的值与左边和上面的值进行比对，如果相同，则说明该位置上对应两字符串中的字符不同(参考之前我们构建二维数组的过程)，有一个位置相同就更新该位置对应的索引(row-- 或者 col--)
- 如果都不同，则说明该位置上两字符串中的字符相同，将其添加到结果集合中即可，并更新row和col
- 如此一来我们就获取答案了
...
- 该题目与leetcode 1143类似，只不过该题目只需要最长公共子序列的长度，不需要具体的字符
- 如果只是求长度的话，我们其实还可以简化一下:
- 根据我们的思路可知: 更新二维数组其实只需要两行就行了，没必要创建一个二维数组
- 所以我们只需要创建两个一维数组，一个作为上一行，一个作为当前行，每当当前行都更新完毕后，将其值复制到上一行即可
- 这样我们就将空间复杂度从O(n*m)降到了O(min(n, m))
...
![Xnip2021-10-18_19-02-51](Algorithem/Xnip2021-10-18_19-02-51.jpg)



![Xnip2021-10-18_20-39-53](Algorithem/Xnip2021-10-18_20-39-53.jpg)



![Xnip2021-10-18_20-16-12](Algorithem/Xnip2021-10-18_20-16-12.jpg)



![Xnip2021-10-18_20-43-02](Algorithem/Xnip2021-10-18_20-43-02.jpg)

题意:

给你一个数组，其元素代表从该位置最大能跳跃的距离，请你计算出从索引0开始到最后一个索引位置的最小跳跃次数
...
- 同样，我们可以创建一个数组，其元素代表从索引0位置到该索引位置所需的最小跳跃次数
- 遍历一次原数组，在每次循环中又从0开始遍历到当前元素位置
- 对于记录数组中每个元素的formula如下:
- if (curIdx <= array[subIdx] = subIdx) (即子数组中元素所能达到的最大索引位置大于当前索引位置)
- jumps[curIdx] = min(jumps[curIdx], jumps[subIdx] + 1) (更新到达当前位置所需的最小步数，从subIdx到curIdx至少需要一步)



复杂度1:

- 我们遍历了一次原数组，每次循环中又从头再遍历了一次，所以时间复杂度为O(n ^ 2)
...
- 创建一个var用来记录答案jumpTime，再创建一个var用来记录下一步可以跳跃到的最大索引位置maxReachIdx，再用一个var记录上一最大索引位置到下一最大索引位置的距离step
- 从索引1开始遍历一次原数组，时刻更新下一可跳跃到的最大索引位置，每次更新都将step递减
- 当step为0后，说明我们此时已经浏览完了上一索引能到达的所有位置，此时必须跳一次，所以jumpTime递增
- 且将step更新: step = maxReachIdx - curIdx
- 最后返回最终的跳跃次数即可
...
![Xnip2021-10-19_08-06-38](Algorithem/Xnip2021-10-19_08-06-38.jpg)



![Xnip2021-10-19_08-41-58](Algorithem/Xnip2021-10-19_08-41-58.jpg)



![Xnip2021-10-19_08-05-33](Algorithem/Xnip2021-10-19_08-05-33.jpg)



![](Algorithem/Xnip2021-10-19_08-34-49.jpg)

题意:

给你一个数组，其中每个位置上的值都代表一根柱子的高度，假如下了一场雨，请计算出这些柱子中能保存的雨水体积
...
- 同样使用动态规划，我们创建一个等长的数组minPillar，其每个元素都代表该位置上能存储的雨水体积
- 每个位置上的体积其实取决于两个因素:
- 其左右最高边界中的最小值，和当前位置的柱子高度
- 所以formula如下:
- minPillar[i] =  min(leftMax, rightMax) - curPillarHeight (如果前者大于后者)
- 最后我们累加每个位置上的体积即可
...
- 创建一个数组minPillar，用来记录每个索引位置对应的边界柱子的最小高度
- 正向遍历一次原数组以获取每个位置对应的左边界最值，保持更新左边界的最高高度
- 反向遍历一次原数组以获取每个位置对应的右边界最值，并与minPillar中保持的数值进行对比，保留较小值(获取左右边界中的较小值)
- 在这一步中可以直接判断原数组中的元素和更新后minPillar中的元素，从而获取每个位置上对应雨水的体积，最后遍历数组进行累加
- 我将这一步延后了一下，其实效果都一样
...
- 我们遍历了两次原数组，又遍历了一次记录数组，所以时间复杂度为O(n)
- 我们创建了一个记录数组，所以空间复杂度为O(n)
...
- 从思路1中可知: 我们需要正向和反向遍历来获取每个位置对应的左右最值，为此我们创建了一个数组
- 但我们其实可以通过双指针的方式来代替
- 如果左指针指向的值小于右指针指向的值，则左边界最值一定小于右边界最值，此时我们可以直接获取左指针指向位置的雨水体积
- 反之我们可以获取右指针指向的雨水体积
...
- 创建两个索引对应左右遍历指针，创建两个变量对应左右最值
- 只要左指针小于右指针就进行如下过程:
- 判断左指针值是否小于右指针值，如果小于则更新左指针(第一个位置没有左边界)，更新左边界最值，累加当前位置的雨水体积
- 如果不小于，则更新右指针(最右位置没有右边界)，更新右边界最值，累加当前位置的雨水体积
- 最后返回累加值即可
...
![Xnip2021-10-20_08-22-11](Algorithem/Xnip2021-10-20_08-22-11.jpg)



![Xnip2021-10-20_08-40-29](Algorithem/Xnip2021-10-20_08-40-29.jpg)



![Xnip2021-10-20_08-49-33](Algorithem/Xnip2021-10-20_08-49-33.jpg)

题意:

给你一个二维数组，其中每个元素都为一个长度为2的数组，代表一个物品，其中的第一个元素为物品的价值，另一个为物品的重量。再给你一个整数，其代表一个袋子能装下物品的最大重量，请你计算出该袋子能装下的物品最大价值之和，并记录下所取的所有物品在原数组中对应的索引值
...
- 同样使用dp，创建一个二维数组，其中col作为当前袋子的最大承重，从0到row对应现有的物品(items中的)，该二维数组用来记录当前容量下能装下现有物品的最大价值
- 用示例数据进行演示，最终该记录数组的右下角的值即为目标容量下能装下物品的最大价值之和
- 其中初始化记录数组中每个元素的formula如下:
- 如果当前位置物品对应的重量 > 当前背包的最大容量，则当前物品无法放入背包，则不会将当前物品放入背包，而取上一次的结果(row - 1):
- 当前值 = 上一行对应的值
- 否则: 当前物品可以放入背包，此时需要判断上次的结果对应的总价值(即不放入当前物品)是否小于放入当前物品后的总价值？
- 其中放入当前物品后的总价值 = 当前物品价值 + (当前背包容量 - 当前物品重量)对应容量的物品价值和
- 最终我们通过遍历该记录数组，获取了最终的最大价值和，但此时还不知道所挑选对应的索引值



- 反推我们的formular，如果在记录数组中，一个元素等于其上一行位置的元素，则说明该位置对应的物品没有被添加到背包中，那么我们就跳过该索引值
- 如果不等于，则说明该位置的物品是被放入背包的物品之一，记录其索引，并更新背包对应的容量
- 最后背包剩余容量为0，则说明我们已经获取了所有物品的索引
...
![Xnip2021-10-21_07-57-51](Algorithem/Xnip2021-10-21_07-57-51.jpg)



![Xnip2021-10-21_08-12-01](Algorithem/Xnip2021-10-21_08-12-01.jpg)

题意: 

给你一个集合，其中每个元素都是一个长度为3的数组，该数组中第一个元素代表一个disk的width，第二个元素代表其depth，第三个元素代表其height，请你从中找出一组严格递增的disk，使它们的height之和最大(严格递增指前一个disk的width, depth, height都要小于后一个才行)
...
- 该题目的思路其实类似Algorithm Day85，区别在于那道题目中的参数为一个简单的数组，每个元素只是一个单纯的数字而已
- 其实抽象一下，比较两个数其实和比较两个数组也没什么差别的
- 所以，我们同样创建一个数组"heights"用来记录当前范围内所有disk严格递增的最大高度之和
- 再创建一个sequence数组用来记录所有heights组合中disk的索引(注意初始化)
- 从索引1位置开始遍历一次输入数组，对应curDisk，每次循环中又从0开始遍历到当前索引前一个位置，对应otherDisk，比较这两个disk判断是否符合条件，符合则判断当前curDisk加上该otherDisk的高度后，是否大于该位置上的最大高度和(heights[curIdx])
- 在遍历中获取最大高度和在heights数组中的索引值maxHeightIdx
- 此时我们仅仅知道了最大的高度和是多少，但不知道其组成元素，其实同Day85中的方法一样



- 通过获取的最大高度和索引，在sequence中获取组成该最大和的所有元素在输入列表中的索引值
- 从maxHeightIdx位置开始从后往前遍历，将每个位置对应的数组添加到sequences中，直到索引对应值为初始值(该位置的数组没有作为和值组成部分被添加)，结束遍历，返回sequences集合
...
- 我们遍历了一次输入集合，每次循环中又从0开始遍历到当前索引位置，所以时间复杂度为O(n ^ 2)
- 我们创建了两个数组和一个集合，所以空间复杂度为O(n)
