...
![Xnip2021-07-14_15-34-08](Algorithem/Xnip2021-07-14_15-34-08.jpg)



![Xnip2021-07-14_15-33-17](Algorithem/Xnip2021-07-14_15-33-17.jpg)

题意:

给你一个矩形二维数组，请你将其中的元素按照螺旋的顺序添加到一个一维数组中，并返回
...
- 抛开实现不谈，很明显我们每次都会由外到内进行一次遍历，而每次遍历后，边缘行和列都会减少
- 而每次一遍历使其都是将四条边上的元素添加到结果集合中
- 所以我们可以使用循环，而每次循环中又包含四个循环，分别对应四条边
- 每次大循环后，我们都更新边界即可
...
- 创建四个变量用于记录边界
- 创建一个while循环，条件为左右边界和上下边界都越界
- 在每次循环中都包含四个循环
- 第一个为添加第一行的元素，行数固定，列数变化
- 第二个为添加最后一列的元素，列数固定，行数变化
- 第三个为添加最后一行的元素，行数固定，列数变化，需要注意的是，如果起始行数和终止行数相同，则跳过这一步(意味着只有一行了)
- 第四个为添加第一列的元素，列数固定，行数变化，如果起始列数和终止列数相同，则跳过这一步(意味着只有一列了)
- 最后返回即可
...
- 我们只对该二维数组遍历了一次，所以时间复杂度为O(n)
- 我们将所有的元素都添加到了一个集合中，所以空间复杂度为O(n)
...
![Xnip2021-07-15_11-34-27](Algorithem/Xnip2021-07-15_11-34-27.jpg)



![Xnip2021-07-15_11-55-15](Algorithem/Xnip2021-07-15_11-55-15.jpg)

题意:

给你一个数组，请你返回其中最长的"山脉"的长度

注: 山脉是指从一个数的两边开始，数值开始减小
...
- 首先，我们可以先找出一个"山脉"的"山顶"，即最大的那个数
- 之后再向两边拓展，直达不符合条件为止，最后再用将两边的索引值相减，则可以得到当前"山脉"的长度
- 每次查找后我们都更新最长的"山脉"
...
- 创建两个变量，一个用来记录最长的山脉长度(从1开始)，一个用来记录当前的索引位置
- 从索引位置开始，对数组进行一次遍历
- 在循环中，首先判断索引位置是否为山顶(通过和两边的值进行比较)，是则进行下一步，不是则更新索引位置
- 从索引位置的两边开始拓展，判断是否为山脉的边界(判断两边是否单调)
- 通过两个边界的索引获取当前山脉的长度，并用来更新最长的山脉长度
- 最后将下一次循环的开始索引位置设置为当前山脉的边界后一位
...
- 我们只对该数组遍历了一次，而其中每次拓展的次数都小于等于n，所以时间复杂度为O(n)
- 我们创建的变量个数与数组长度无关，所以空间复杂度为O(1)
...
![Xnip2021-07-16_14-42-14](Algorithem/Xnip2021-07-16_14-42-14.jpg)



![Xnip2021-07-16_14-43-56](Algorithem/Xnip2021-07-16_14-43-56.jpg)



![Xnip2021-07-16_15-08-43](Algorithem/Xnip2021-07-16_15-08-43.jpg)



![Xnip2021-07-16_14-51-59](Algorithem/Xnip2021-07-16_14-51-59.jpg)

题意:

给你一个数组，请你返回一个等长度的数组，其中每个元素的值都为原数组中除该位置外所有元素的乘积
...
- 将数组遍历一次，在每次循环中，我们可以再从头对数组进行一次遍历
- 将其中所有的索引位置与当前位置不重合的值乘起来，结果保存在结果数组的相同位置即可



实现1:

- 首先创建一个长度和参数数组一样的结果数组
- 将参数数组遍历一次，考虑到Java的特性，将结果数组对应位置的值初始化为1
- 在其中再写一个for循环，将所有索引值不等于当前索引的值累乘到结果数组中相同的位置
- 最后返回结果数组即可
...
- 我们将数组遍历了n次，n为数组的长度，所以时间复杂度为O(n^2)
- 我们创建了一个和参数数组等长的结果数组，所以空间复杂度为O(n)
...
- 将前面同方法一相同，将参数数组遍历一次
- 在循环中，我们取当前索引位置的左一位和右一位
- 通过这两个索引向两边拓展，并将各个位置的值累乘至结果数组的当前索引位置
...
- 首先创建一个长度和参数数组一样的结果数组
- 将参数数组遍历一次，考虑到Java的特性，将结果数组对应位置的值初始化为1
- 在每次循环中，我们都取当前索引位置的左一位和右一位作为向两边拓展的索引
- 将左右索引向两边更新，并将对应的值都累乘到结果数组的当前索引位置
- 最后返回结果数组即可
...
- 我们参数数组遍历了一次，而每次循环中，我们都用两个索引将剩余的n-1个位置遍历了一次，所以总用时为n(n-1)，时间复杂度为O(n^2)
- 我们创建了一个和参数数组等长的结果数组，所以空间复杂度为O(n)
...
- 从1开始将每个索引位置的值都更新为累乘的结果，如：

Array:[5,1,4,2]

累乘后:[1,5,5,20]

- 再从1开始，反向更新每个位置值为累乘结果，如:
...
- 首先创建一个长度和参数数组一样的结果数组
- 创建两个变量，并初始化为1
- 先正向遍历一次，将结果数组每个位置的值都设置为其中一个变量，之后将参数数组每个位置的值累乘到该变量中
- 再逆向遍历一次，将结果数组中每个位置的值都累乘另一个变量，之后将参数数组每个位置的值累乘到该变量中
- 最后返回即可
...
![Xnip2021-07-17_10-55-06](Algorithem/Xnip2021-07-17_10-55-06.jpg)



![Xnip2021-07-17_10-46-00](Algorithem/Xnip2021-07-17_10-46-00.jpg)



![Xnip2021-07-17_10-50-07](Algorithem/Xnip2021-07-17_10-50-07.jpg)



![Xnip2021-07-17_10-53-48](Algorithem/Xnip2021-07-17_10-53-48.jpg)

题意

给你一个数组，请你找出第一个重复的元素，如果没找到则返回-1
...
- 首先题目要求是第一个重复的元素，所以我们只能从左到右进行遍历
- 首先使用最常规的方法:对每个元素都进行验证
- 即对数组进行一次遍历，之后再用当前元素和它右边剩下的元素进行比较即可，一旦找到则记录下它的索引值
- 由于通过这种方式找到的第一个值不一定是第一个重复的元素索引，所以我们需要时刻记录最小的重复值索引



实现1:

- 创建一个变量用来记录最小的元素值索引，并赋予一个较大的初始值
- 遍历参数数组，在其中再写一个从右边一位索引开始的循环遍历，比较剩余元素和当前元素的值
- 时刻更新重复元素的索引值
- 最后如果该记录值和初始值一样，则说明没有重复值，返回-1即可，否则返回记录值对应的元素



复杂度1:

- 在最坏的条件下，我们需要将该数组遍历n次，n为该数组的长度，所以时间复杂度为O(n^2)
- 我们创建的变量数为常数与n无关，所以空间复杂度为O(1)
...
- 哈希表中有一个特性: 其中的元素不能重复
- 我们可以利用这个特性，将所有的元素都添加进去，一旦有一次添加失败，则说明哈希表中已经存在该元素，直接返回该元素即可



实现2:

- 创建一个HashSet集合，将参数数组遍历一次，并将所有的元素都添加到哈希表中
- 利用"add()"方法的返回值，如果返回值为false，则说明该元素以及存在，直接返回即可



复杂度2:

- 我们只遍历一次参数数组，所以时间复杂度为O(n)
- 我们创建了一个HashSet集合，最坏的情况下，我们需要存储n个元素，所以空间复杂度为O(n)
...
- 既然重复的数在数值上一样，那么将它们作为索引值，对应的元素位置也应该是唯一的
- 我们将数组遍历一次，每次都将该数值作为索引值，将该位置对应的值进行标记(比如标记为负数)
- 下一次再遍历到相同元素时，对应位置的值如果已经被标记，则说明该值为重复值，直接返回即可



实现3:

- 对参数参数进行一次遍历，将每次的元素值当作索引值，将索引位置上的数值标记为负数
- 如果该位置上的值已经为标记了，则直接返回即可



复杂度3:

- 我们只对该数组遍历了一次，所以时间复杂度为O(n)
- 我们创建的变量数为常数与n无关，所以空间复杂度为O(1)
...
![Xnip2021-07-18_17-06-23](Algorithem/Xnip2021-07-18_17-06-23.jpg)



![Xnip2021-07-18_17-05-15](Algorithem/Xnip2021-07-18_17-05-15.jpg)

题意:

给你一个二维数组，其中每个元素都为一个含有两个元素的数组，每个元素都被视为一个区间

请你将其中重合的区间进行合并，并最终返回所以互不重合的区间



思路:

- 既然谈到区间，那我们完全可以将这些数组区间全部映射到"数轴"上
- 略加注意就会发现:
- 如果两个区间有重合的部分，那么前一个区间的右边界一定大于后一个区间的左边界，我们可以利用这一点进行判断



实现:

- 因为这些区间在数组中不一定是有序的，所以我们先进行排序，在Java里对二维数组进行排序会用到lambda表达式
- 这里有一点比较特殊:
- 不管第一个数组区间是否与之后的区间有所重合，第一个区间的左边界一定是在结果中的，不确定的只有右区间而已
- 因此，我们将第一个数组先添加到结果集合中，之后对二维数组进行遍历
- 每次遍历，都将当前数组区间的右边界与下一个数组区间的左边界进行比较
- 如果大于等于，则拓展当前数组的右边界
- 如果小于，则将当前数组添加到结果集合中
...
- 我们首先对二维数组进行了一次排序，花费为nlogn，n为二维数组的区间数量
- 之后又对二维数组进行了一次遍历，花费为n
- 所以时间复杂度为O(nlogn)



- 我们创建了一个集合用来存储所以不重复的数组区间，最坏时会消耗的空间为n，所以空间复杂度为O(n)
...
![Xnip2021-07-19_18-12-36](Algorithem/Xnip2021-07-19_18-12-36.jpg)



![Xnip2021-07-19_18-12-25](Algorithem/Xnip2021-07-19_18-12-25.jpg)

题意:

给你一个数组，其可能是有序的，也可能是无序的。

如果是有序的，请返回数组[-1, -1]；如果是无序的，请你找出将其中哪个索引范围内的元素进行排序后就能使数组有序，并以数组的方式返回这个索引范围
...
- 通过观察可以发现一个规律:
- 在被排序的部分中，数值最小的数字和数值最大的数字在进行排序后，其一定处于所需排序范围的开头和结束索引位置处
- 所以我只需要找出在所需排序范围内的最值，然后找到它们俩在排序时该插入的索引位置，即是我们所需的答案
...
- 首先我们需要对数组进行一次遍历，并判断每个元素是否有序，通过与其左右的元素进行比较(除了第一个和最后一个元素)
- 如果是无序的，则将其与我们的记录值进行比较，时刻更新无序元素中的最值
- 一次遍历后进行判断: 最值是否与初始值相同？如果相同，则说明所有元素都为有序，直接返回数组[-1, -1]即可。如果不是则进行下一步:
- 对数组进行一次遍历，找到我们记录的最小值应该插入的索引位置，记录后跳出循环
- 对数组进行一次倒序遍历，找到我们记录的最大值应该插入的索引位置，记录后跳出循环
- 最后返回一个含有最小值和最大值的数组即可



复杂度:

- 我们对数组进行了三次遍历，所用时间为3n，n为数组的长度，所以时间复杂度为O(n)

- 我们创建的变量个数是常量级的，所以空间复杂度为O(1)
...
![Xnip2021-07-20_13-16-36](Algorithem/Xnip2021-07-20_13-16-36.jpg)



![Xnip2021-07-20_13-30-13](Algorithem/Xnip2021-07-20_13-30-13.jpg)

题意:

给你一个数组，请你找出其中所具有的最长的连续数字区间，并以该区间的起始和结束位置的数字为元素返回一个长度为2的数组

例子：

输入数组: [1, 11, 3, 0, 15, 5, 2, 4, 10, 7, 12, 6]

结果: [0, 7]

解释: 该数组中具有从0到7的全部数字，而该区间是最长的
...
- 因为我们要找的是最长的连续区间，所以每个数字都需要判断其是否存在与数组中
- 我们可以先对数组进行排序，然后进行遍历，判断是否连续，一旦中断就更新最长的区间和对应的开始和结束位置的数字，这样做是没问题的，也很容易想到
- 但这样做的时间复杂度为O(nlogn)，并不是最佳解，面试官并不会满意，而且如果面试官要求不得更改原数组的话，我们还得创建额外的空间才行



- 判断一个数字是否存在于数组中其实可以利用集合的contains()方法，所以我们只需要将数组中元素都转移到一个集合中即可
- 之后再以每个元素为起点，从两边开始拓展，一直拓展到不存在为止，然后我们就找到了一个连续的区间和它对应的开始和结束位置的数字，最后再更新最大区间和对应的起始结束数字即可
- 但这里还有一点问题: 在对每个元素都进行拓展时，最终的结果很有可能是我们已经得到了的(拓展出来的区间是重复的)，为了避免重复，我们可以利用每个元素的独立性，在拓展的时候就对拓展的数字进行标记
- 这样一旦下一次开始拓展的元素是被标记过的话，我们就直接跳过，节省时间
...
- 为了能够使用集合中的contains()方法，我们需要对应数组进行一次遍历，并将其添加到一个集合"recorder"中
- 这里还需要创建两个变量，来获取该数组中的最大最小值(之后有用)
- 由于我们需要对数组中已经拓展过的元素进行标记，所以我们创建一个boolean数组"processed"
- 由于该数组的长度需要包含参数数组中的所有数字，所以我们将该boolean数组的长度设置为最大值到最小值的区间(max - min + 1)
- 这里就需要用到之前我们获取的源数组的最大最小值了
- 这样一来，只要我们之后判断出来一个数字在源数组中，就将"processed"中对应索引位置(当前数字 - 最小值)的值修改为true就完成了对该数字的标记，避免了重复
- 再对源数组进行一次遍历(其实对recorder集合进行遍历也是可以的)，首先判断当前的元素是否被标记过(processed[curNum - min])，如果已经被标记了，则直接跳过，没有则进行一下步骤:
- 以当前元素为起点，向两边拓展，如果拓展后的边界存在于"recorder"中，则将其进行标记，并继续拓展，直到两边都不存在为止
- 这样我们就得到了一个连续的区间(curLeft + 1 到 curRight - 1)
- 将其与最大的连续区间进行对比以更新最大的连续区间
- 最后返回最大的连续区间即可
...
- 我们对该数组完整遍历了两次，在第二次遍历中，我们拓展的次数和第二次遍历是相关的，所以总时间是2n，n为数组的长度，所以时间复杂度为O(n)
- 我们创建了一个集合和一个boolean数组，最坏的情况下我们会消耗2n的空间，所以空间复杂度为O(n)
...
![Xnip2021-07-24_22-04-48](Algorithem/Xnip2021-07-24_22-04-48.jpg)



![Xnip2021-07-24_21-54-11](Algorithem/Xnip2021-07-24_21-54-11.jpg)
...
## Tag: Linked List, Difference value



![Xnip2021-07-25_22-21-09](Algorithem/Xnip2021-07-25_22-21-09.jpg)



![Xnip2021-07-25_22-20-44](Algorithem/Xnip2021-07-25_22-20-44.jpg)



![Xnip2021-07-25_22-20-55](Algorithem/Xnip2021-07-25_22-20-55.jpg)

题意:

给你两个链表的表头，请你找出两个链表中第一个重合的节点



思路:

- 两个链表节点重合其实就是两个节点对象相同，其后的所有节点都共用，但两个链表的长度并不一定是相同的
- 这里我们需要对链表进行"右对齐":
- 首先计算两个链表的长度，然后先开始遍历较长的那个，等到两个链表的长度一样时，再比较每个节点即可
...
![Xnip2021-07-26_15-18-45](Algorithem/Xnip2021-07-26_15-18-45.jpg)



![Xnip2021-07-26_15-21-02](Algorithem/Xnip2021-07-26_15-21-02.jpg)



![Xnip2021-07-26_15-21-18](Algorithem/Xnip2021-07-26_15-21-18.jpg)



![Xnip2021-07-26_15-21-12](Algorithem/Xnip2021-07-26_15-21-12.jpg)

题意:

给你一个字符串，请你判断其是否为对称的回文字符串
...
- 创建两个指针分别代表字符串的两边
- 设立一个循环，判断两边的字符是否相等，一旦不同，则参数字符串不为回文字符串
...
- 我们通过双指针的方式将参数字符串遍历了一次，所以消耗的时间为n，n为字符串的长度(字符数)，时间复杂度为O(n)
- 创建的变量为常量级，所以时间复杂度为O(1)
...
- 我们完全可以将原字符串与将其反转后的结果进行比较，所以我们只需要得到一个反转的字符串即可



实现2:

- 在java里，我们可以利用StringBuilder类里的reverse()方法快速便捷的将原字符串进行反转，再进行对比
- 如果不熟序StringBuilder类，也可以手动将倒序的字符添加到新的字符中以构建反转的字符串(对原字符进行倒序遍历即可)



复杂度2:

- 我们只对原字符串遍历了一次，所以时间复杂度为O(n)
- 我们创建了一个变量以存储反转后的字符串，所以空间复杂度为O(n)
