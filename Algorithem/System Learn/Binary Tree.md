...
![Xnip2022-06-23_08-02-44](Binary Tree/Xnip2022-06-23_08-02-44.jpg)
...
- 而一颗二叉搜索树通过中序遍历后，节点值一定是严格递增的，因此我们只需要对该二叉树进行一次中序遍历，遍历的方法可以是递归，也可以通过栈来迭代
...
- 在比较时，我们其实只需要记录最后一个数，所以我们只需要维护最后一个数即可，不需要创建一个集合来记录所有的节点
...
![Xnip2022-06-23_08-01-49](Binary Tree/Xnip2022-06-23_08-01-49.jpg)
...
![Xnip2022-06-22_08-03-53](Binary Tree/Xnip2022-06-22_08-03-53.jpg)



![Xnip2022-06-22_08-12-06](Binary Tree/Xnip2022-06-22_08-12-06.jpg)



![Xnip2022-06-22_08-12-58](Binary Tree/Xnip2022-06-22_08-12-58.jpg)
...
![Xnip2022-06-24_08-20-07](Binary Tree/Xnip2022-06-24_08-20-07.jpg)
...
![Xnip2022-06-21_07-42-27](Binary Tree/Xnip2022-06-21_07-42-27.jpg)



![Xnip2022-06-21_07-41-16](Binary Tree/Xnip2022-06-21_07-41-16.jpg)

题意:

给你一个数组，请你根据它构建出一颗最大二叉树(根结点是数组中的最大值，左子树中的值是最大值左边的值，右子树中的结点值是数组最大值中右边的值，子树的根结点值是子树中的最大值)
...
- 该题目无非就分解为两步: 获取最大值，递归构造
- 其中求最大值可以直接遍历获取，递归构造的时候就需要注意边界了
- 而每次遍历的边界自然要根据根结点在数组中的索引值来获取，通过左右边界索引即可判断当前子树中的剩余结点数，如果为0则返回null，为1就直接构造返回对应的结点
...
- 我们每次构造子树中的结点时都需要遍历数组，所以时间复杂度为O(n ^ 2)
- 我们递归构造树需要调用栈空间，其取决于树的高度，所以空间复杂度为O(n)
...
![Xnip2022-06-24_07-43-05](Binary Tree/Xnip2022-06-24_07-43-05.jpg)



![Xnip2022-06-24_07-40-59](Binary Tree/Xnip2022-06-24_07-40-59.jpg)

题意:

给你一颗二叉搜索树，请你返回其中任意两个结点之间的最小差值



思路:

- 因为是二叉搜索树，所以这些结点按照中序遍历的方式进行遍历的话，所有的结点值都是有序的，这样一来差值最小的结点一定是紧邻的
- 所以我们只需要进行一次中序遍历，即可获取最小差值



复杂度:

- 我们遍历了一次树中的结点，所以时间复杂度为O(n)
- 我们创建了一个栈来模拟迭代的过程，所以空间复杂度为O(n)
...
![Xnip2022-06-25_10-43-02](Binary Tree/Xnip2022-06-25_10-43-02.jpg)



![Xnip2022-06-25_10-43-44](Binary Tree/Xnip2022-06-25_10-43-44.jpg)
...
![Xnip2022-06-26_08-29-28](Binary Tree/Xnip2022-06-26_08-29-28.jpg)



![Xnip2022-06-26_08-40-29](Binary Tree/Xnip2022-06-26_08-40-29.jpg)

题意:

给你一个二叉搜索树，一个整数，请你用该整数构建一个结点，并将该结点插入到二叉搜索树中对应的位置处
...
- 最简单的方法就是通过递归的方式搜索不同的分支，即当前结点如果小于参数值则递归调用右子树，反之调用左子树，直到当前结点为null时返回构造的新结点
- 我们可以通过循环来模拟上述递归过程，不过需要设置一个前驱结点来锚定最后新结点插入的位置
- 搜索过程依然是用当前结点与目标值进行对比来更新当前结点是往左还是右，最后当前结点为null时，根据前驱结点和目标值的大小关系确定插入位置即可
...
![Xnip2022-06-27_08-13-33](Binary Tree/Xnip2022-06-27_08-13-33.jpg)



![Xnip2022-06-27_08-26-15](Binary Tree/Xnip2022-06-27_08-26-15.jpg)

题意:

给你一个二叉搜索树，一个目标结点值，请你将值为该目标值的结点进行删除，并维护处理后的树
...
- 因为是二叉搜索树，所以当前结点与目标值不相等时，根据大小向对应的子树中去搜索即可
- 当搜索到对应结点后，则需要分情况讨论:
- 如果为叶子结点，那么直接返回null即可
- 如果其中一个子结点为null，那么返回另一个不为null的即可
- 如果都不为null，则需要先获取右子树中的最左下左结点，然后将目标结点的左子树作为该最左下左结点的左子树即可



复杂度:

- 我们需要遍历树中的结点，所以时间复杂度为O(n)
- 我们递归遍历使用的栈空间取决于树的高度，所以空间复杂度为O(n)
...
![Xnip2022-06-28_07-50-59](Binary Tree/Xnip2022-06-28_07-50-59.jpg)



![Xnip2022-06-28_08-09-51](Binary Tree/Xnip2022-06-28_08-09-51.jpg)

题意:

给你一颗二叉搜索树，两个数字，这两个数字代表一个区间，请你将树中结点值不在区间内的结点全部剪掉了，并返回重构的二叉树



思路:

- 所谓的剪掉其实就是删除该结点而已，不过因为这里是二叉搜索树，所以有更好的方法可以解决
- 假如当前结点值小于low，那么其左子树必然都会剪掉，所以此时我们直接返回其右子树即可，同理如果大于high则返回其左子树
- 所以所谓的剪枝操作其实就是重链接其子树而已
...
![Xnip2022-06-30_16-32-54](Binary Tree/Xnip2022-06-30_16-32-54.jpg)



![Xnip2022-06-30_16-36-54](Binary Tree/Xnip2022-06-30_16-36-54.jpg)

题意:

给你一颗二叉搜索树，请你将其所有的结点值修改为等于原树中大于或等于该结点原值的结点值之后



思路:

- 因为这颗树为二叉搜索树，如果将其看作一个数组，那么该题目就可以看作是将一个有序数组的元素从末尾到开头进行累加
- 其中中序遍历是从头到尾，那么中序遍历的反向不就是从尾到头了吗？所以我们只需要进行一次中序遍历的反向遍历即可
