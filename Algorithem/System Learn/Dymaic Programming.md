...
- 但这里还不太一样，其实还应该加上源字符串前一个范围与当前目标字符串进行匹配时的子序列数量，这样才算完整
...
![Xnip2022-05-13_07-19-07](Dynamic Programming/Xnip2022-05-13_07-19-07.jpg)



![Xnip2022-05-13_07-31-33](Dynamic Programming/Xnip2022-05-13_07-31-33.jpg)

题意:

给你一个字符串，请你找出其中最长回文子序列的长度
...
- 注意，这里需要的是回文子序列，与回文子串不同的是，回文子序列里的字符不需要连续，只需要保持在源字符串里的相对位置即可
- 但方法其实和昨天一样，我们依然使用一个二维数组来表示对应范围内的字符串中的最长回文子序列长度
- 进入5部曲:
- 第一步: 创建一个二维数组dp[left][right]，其中left代表其实位置索引，right代表结尾位置索引，该元素代表从left到right位置处，最长的回文子序列的长度
- 第二步: 在匹配的时候无非就两种情况:
- 1.左右索引指定的字符是相等的，那么此时该范围内的回文子序列长度应该为其内部字符串的回文子序列长度 + 2，内部字符串的回文子序列长度可用dp[left + 1][right - 1]表示
- 2.左右索引指定的字符不相等，此时需要从其他位置获取子序列长度，要么保留右边界，要么保留左边界，所以需要在dp[left + 1][right]和dp[left][right - 1]两个元素中取最大值
- 所以递推公式为:
...
- 第三步: 因为我们用两个索引代表左右边界位置，所以难免会出现两个索引指向同一个位置的情况，此时对应范围内只有一个字符，但其同样是回文字符，且长度为1，所以我们需要将这些位置的元素都初始化为1(即对角线上的元素)
- 第四步: 从递推公式可见，我们需要下一行和前一列的数据，所以在遍历行的时候需要倒序，而列则正常顺序即可
- 最后简单推导
...
- 我们遍历了该dp数组，所以时间复杂度为O(n ^ 2)
- 我们创建了一个dp数组，所以空间复杂度为O(n ^ 2)
