...
![Xnip2022-05-31_08-18-39](String/Xnip2022-05-31_08-18-39.jpg)
...
- 之后，创建一个活动的指针，从右边开始，直到活动指针指向的字符为空格(活动指针的右边界即为我们的单词)，将该范围内的字符倒序添加到结果字符中
...
- 最基本的思路就是反转整个字符串，再反转其中的每个单词
- 不过在反转之前，我们必须要先处理字符串中的空格
- 左右两边字符串外的空格使用两个指针即可去除
- 中间多余空格则需要使用StringBuilder来去除，只需要保证末尾有空格的情况下添加字符，没有空格的情况下可以添加空格即可
- 处理完空格后，我们只需要反转整个字符串，再通过双指针翻转每个单词即可
...
![Xnip2022-06-01_07-58-55](String/Xnip2022-06-01_07-58-55.jpg)
...
![Xnip2022-02-17_15-12-56](String/Xnip2022-02-17_15-12-56.jpg)



![Xnip2022-02-17_15-18-31](String/Xnip2022-02-17_15-18-31.jpg)

题意:

给你一个字符串，请你返回其中第一个唯一字符对应的索引，没有唯一字符则返回-1
...
- 判断唯一的方法无非就是判断其出现的次数，所以这还是一道词频统计的题目
- 因为只有小写字母，所以这里我们创建一个长度为26的数组就能对词频进行统计了
- 统计后再根据顺序判断每个字符，一旦出现词频为1的，那么直接返回当前索引即可
...
- 我们创建的词频统计数组的长度是固定的(26)，所以空间复杂度为O(1)
...
![Xnip2022-02-19_10-54-52](String/Xnip2022-02-19_10-54-52.jpg)



![Xnip2022-02-19_10-59-10](String/Xnip2022-02-19_10-59-10.jpg)

题意:

给你一个目标字符串，一个子串，请你判断该子串是否存在于该目标字符串中
...
- 该题目其实和Day205的做法差不多，都是KMP算法，但这的返回值是boolean类型，所以在返回值上需要做一些改动
...
![Xnip2022-02-20_16-29-46](String/Xnip2022-02-20_16-29-46.jpg)



![Xnip2022-02-20_17-05-04](String/Xnip2022-02-20_17-05-04.jpg)

题意:

给你一个只有'('，')'，'*'三种字符组成的字符串，请你判断该字符串括号是否有效
...
- 如果没有'*'，那么匹配括号用栈就行，但这里引入了星号
- 这里只需要在匹配的时候加一个逻辑: 先判断是否有左括号，有则用左括号匹配，没有则判断是否有星号，有则匹配，还是没有则返回false
- 遍历完所有字符后，我们再判断是否有剩余的左括号和星号，一次弹出两个栈的栈顶元素进行比较，如果左括号对应索引大于星号，则返回false(左括号在星号右边)



复杂度:

- 我们遍历了一遍所有的字符，所以时间复杂度为O(n)
- 我们创建了两个栈，所以空间复杂度为O(n)
