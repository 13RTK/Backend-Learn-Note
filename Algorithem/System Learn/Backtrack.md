...
![Xnip2022-07-03_08-27-16](Backtrack/Xnip2022-07-03_08-27-16.jpg)
...
> 如果是集合不能重复，那就是树层去重，这里只需要在排序后跳过有效范围内临近的重复元素即可

- 递归调用即为树中的分支向下，for循环即为同层中的横向节点

![Xnip2022-07-05_10-33-20](Backtrack/Xnip2022-07-05_10-33-20.jpg)
...
![Xnip2022-04-10_09-08-49](Backtrack/Xnip2022-04-10_09-08-49.jpg)



![Xnip2022-04-10_09-25-22](Backtrack/Xnip2022-04-10_09-25-22.jpg)

题意:

给你一个由数字和字母组成的字符串，请你尝试将其中的字母大小写进行改变从而获取所有可能的新字符串排列



思路:

- 很明显，我们需要遍历整个字符串，一旦遇到数字则需要跳过，否则转换其大小写
- 转换大小写有一个tip: 在ASCII中，每对大小写字母之间的二进制数只有第6位有差别(即相差32)，所以我们只需要与空格字符' '进行异或运算即可转换大小写了
- 剩下的就是回溯的老套路了
...
![Xnip2022-07-13_12-23-00](Backtrack/Xnip2022-07-13_12-23-00.jpg)



![Xnip2022-07-13_12-17-53](Backtrack/Xnip2022-07-13_12-17-53.jpg)



![Xnip2022-07-13_12-21-17](Backtrack/Xnip2022-07-13_12-21-17.jpg)

题意:
给你一个表示机票的列表，每个元素都是一个长度为2的列表，第一个元素代表出发地，第二个元素代表目的地，请你对该行程进行重新规划排序，且行程必须从JFK开始，每张机票只能用一次，最终得到的行程如有多个，则返回按照字典序排序的那个
...
- 乍一看确实没有头绪，这里我们可以先将每张机票中的出发地和目的地关联起来，这里用一个Map即可，且需要注意的是: 该Map对应的value记录了每个目的地出现的次数
- 做完这一步之后，就是获取行程的步骤了
- 因为行程默认从JFK开始，所以我们先将其添加到结果集合中(这里用了双端队列，因为下一个目的地取决于最后一个元素)
- 一旦结果集合的size = 机票数 + 1，则说明该结果满足条件，但仅仅是这样还无法解决字典序的问题，因此我们在之前填充Map的时候，还必须要将value设置为有序的map，这里我们用到了TreeMap
- 在搜索行程的时候，我们每次考虑下一个行程时，自然是从当前行程的最后一个地点开始的，所以需要搜索最后一个行程是否出现在之前的map中(即是否有从最后一个地点开始的机票)
- 如果没有，则当前行程自然作废
- 如果有，且机票数量足够，则将其对应的目的地添加到结果集合中，并将当前机票对应的数量 - 1，如果机票数量不够则一样作废
- 最后返回这个集合即可
