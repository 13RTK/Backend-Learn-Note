...
## Tag: Binary Tree, DFS, Preorder Traversal

![Xnip2021-09-15_07-35-40](Algorithem/Xnip2021-09-15_07-35-40.jpg)



![Xnip2021-09-15_07-49-55](Algorithem/Xnip2021-09-15_07-49-55.jpg)



![Xnip2021-09-15_07-50-09](Algorithem/Xnip2021-09-15_07-50-09.jpg)
...
![Xnip2021-09-16_07-48-56](Algorithem/Xnip2021-09-16_07-48-56.jpg)



![Xnip2021-09-16_07-56-19](Algorithem/Xnip2021-09-16_07-56-19.jpg)

题意:

给你一个字符二维数组，其中字符'1'代表陆地，'0'代表水域，其中如果几个'1'相连则视作一片岛屿(仅限上下左右)，请你计算出岛屿的数量
...
![Xnip2021-09-17_12-33-15](Algorithem/Xnip2021-09-17_12-33-15.jpg)



![Xnip2021-09-17_14-51-44](Algorithem/Xnip2021-09-17_14-51-44.jpg)
...
![Xnip2021-09-18_07-45-29](Algorithem/Xnip2021-09-18_07-45-29.jpg)



![Xnip2021-09-18_07-53-32](Algorithem/Xnip2021-09-18_07-53-32.jpg)
...
- 同Algorithm Day55，同样是四向的dfs，只是从简单的计数变成了累加而已其余不变
...
![Xnip2021-09-19_07-22-56](Algorithem/Xnip2021-09-19_07-22-56.jpg)



![Xnip2021-09-19_07-22-27](Algorithem/Xnip2021-09-19_07-22-27.jpg)



![Xnip2021-09-19_07-50-23](Algorithem/Xnip2021-09-19_07-50-23.jpg)
...
![Xnip2021-09-20_07-26-44](Algorithem/Xnip2021-09-20_07-26-44.jpg)



![Xnip2021-09-20_07-25-40](Algorithem/Xnip2021-09-20_07-25-40.jpg)



![Xnip2021-09-20_07-42-01](Algorithem/Xnip2021-09-20_07-42-01.jpg)



题意:

给你一个排序后的数组和一个目标值，请你找出该目标值在数组中所处的索引值，如果没有找到则返回-1
...
- 简单的将整个数组遍历一次当然可以找到，此时时间复杂度为O(n)
- 但这里的数组经过了排序，所以我们可以做得更好，即使用二分查找
- 二分查找的前提是数组经过了排序，在使用时，只需要每次都取中间那位数
- 将该中间数与目标值进行比较，如果大于目标值说明中间数在目标数的左边，需要往右，所以我们舍弃掉左边的部分
- 此时需要更新左边界，同理如果大于则更新右边界，直到找到目标或者左右边界大小变化
...
- 其实在Arrays类中为我们提供了一个方法就叫binarySearch()，直接用是没问题的，不过该方法如果无法找到目标值时不会返回-1，所以需要一步处理
- 根据思路，我们将二分法实现一次: 首先定义左右边界，刚开始没有任何一个部分被舍弃，所以我们的左右边界需要包含所有区域，所以索引0和array.length - 1分别为左右边界
- 通过两者的中间索引值获取数组的中间值，并与目标值比较，根据大小关系更新对应的边界值
...
![Xnip2021-09-21_08-11-11](Algorithem/Xnip2021-09-21_08-11-11.jpg)



![Xnip2021-09-21_08-21-51](Algorithem/Xnip2021-09-21_08-21-51.jpg)



![Xnip2021-09-21_08-25-35](Algorithem/Xnip2021-09-21_08-25-35.jpg)

题意:

给你一个未排序的数组，请你找出其中三个最大的元素，并将它们以数组的形式返回(从小到大的顺序)



思路:

- 比较获取数组中的含顺位的值自然会想到对数组进行排序，然后根据索引获取需要的元素
- 这样当然可以，但时间复杂度为O(nlog(n))，能不能不靠排序解决这个问题呢？
- 在排序的做法中，我们是先找出目标元素，之后再构建结果数组的，那能不能先构建数组，之后动态的更新这个数组呢？
- 按照这样的思路，我们创建一个长度为3的数组，初始化其中的值为最小int值，之后遍历一次参数数组
- 动态的更新结果数组中的元素，按照大小顺序，我们肯定会想到需要先更新最后一个元素，之后再更新剩下两个位置



实现(Java):

- 创建一个数组，初始化值为Integer.MIN_VALUE，遍历参数数组
- 将当前数字从结果数组的最后一个元素开始依次比较，并更新
- 这里有一个问题，如果我们只是单纯的用新值覆盖旧值，那么原来位置上的值就会消失，但这个消失的值有可能是剩下两个位置的最终结果，通过这样的方式被我们舍弃了
- 因此，在更新前，我们需要将旧值移动到前一个索引位置才行，避免其被舍弃，这就是Solution Ver.2中updateArray方法的作用
- 最后返回该结果集合即可
...
- 我们只将该数组遍历了一次，所以时间复杂度为O(n)
- 我们只创建了一次长度为3的数组，其与n无关，所以空间复杂度为O(1)
