...
## Tag: Binary Tree, Path Sum, DFS

![Xnip2021-11-04_08-16-24](Algorithem/Xnip2021-11-04_08-16-24.jpg)



![Xnip2021-11-04_08-16-04](Algorithem/Xnip2021-11-04_08-16-04.jpg)

题意:

给你一颗二叉树，一个目标数，请你计算出该二叉树中从任意位置开始的路径和等于该目标数的路径数量
...
- 与前两天的题目不同的是，这道题目的路径和不再限制路径为从根节点到叶子节点的路径了，所以情况也复杂一些
- 在二叉树中有一个概念: 前缀和。也就是从根节点到当前节点的所有节点值之和
- 如果有两个不同的节点a和b，他们之间一些节点的和等于目标数的话，其实就相当于: 节点a的前缀和 - 节点b的前缀和 = 目标数
- 比如题目中左子树节点3的前缀和为18(3 + 5 + 10)，而根节点的前缀和为10，所以他们之间的前缀和值之差就为目标数了
- 再将这个等式换一下位置: 节点a的前缀和 - 目标数 = 节点b的前缀和，所以我们只需要记录所有节点以不同节点为根节点的前缀和出现的次数
- 一旦重复出现了前缀和 = 之前节点的前缀和 - 目标数，则递增我们的结果，最后返回结果即可
- 当取到叶子节点后记得要删去一次前缀和的记录，避免之和重复计算(该分支的路径已经计算完成)
...
- 我们创建了一个hashmap来记录前缀和，所以空间复杂度为O(n)
