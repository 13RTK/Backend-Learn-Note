...
- 请求的数据是数据里没有的，所以没有出现在缓存中，需要到数据库中查询
- 而数据库中也无法查询到，所以就会一直对数据库进行请求，这样缓存就失效了

![Xnip2022-05-10_13-14-09](Redis basic.assets/Xnip2022-05-10_13-14-09.jpg)



解决方法:

1. 每次在数据库中的查询结果为空时，则让Redis缓存这个为空的结果，并设置合理的过期时间
2. 设置一个布隆过滤器
...
![Xnip2022-05-10_13-18-31](Redis basic.assets/Xnip2022-05-10_13-18-31.jpg)



- 布隆过滤器本质上是一个存储二进制值的数组

先将数据库中的每个数据/键分成三份计算hash值，其中每个hash值都表示数组中的一个位置，将对应的位置设置为1

之后进行查询的时候，计算这个键对应的三个hash值，查看三个位置上的值

> 如果都为1，则说明该条数据可能存在
>
> 如果有一个为0，则说明该条数据一定不存在
...
- 某一时间，缓存中的某个，某些key都失效了
- 然而突然对这些key的访问量激增，然而缓存中没有这些数据，因此会全部去向数据库请求数据，相当于缓存被击穿了

![1f307ba02e1c1c04c73b877e167d58e6](Redis basic.assets/1f307ba02e1c1c04c73b877e167d58e6.webp)



解决方法:

- 合理设置key的过期时间，不让大量的key同时过期
...
- 同一时间大量的key过期或者redis服务器崩溃，此时会有大量的请求发送给数据库

![Xnip2022-05-10_13-31-05](Redis basic.assets/Xnip2022-05-10_13-31-05.jpg)



解决方法:

创建Redis服务器集群(高可用)，设置一些服务熔断降级机制
