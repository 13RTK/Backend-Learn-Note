...
- 如果只将Buffer Pool中的页面进行了修改，**还未刷新脏页**，此时**如果事务提交出现了错误，内存中的数据丢了**，那么事务所做的修改也丢失了，这是不应该的
...
- MLOG_WRITE_STRING比较特殊，其需要写入一个字节序列(len)，因为不能确定写入的具体数据占用多少字节，所以需要在日志中添加一个len字段
...
SQL语句执行后会更新聚簇索引/二级索引对应的B+树中的页，但**这些修改都发生在Buffer Pool中，所以需要记录下对应的redo日志**
...
为了管理redo日志，一个MTR生成的redo日志都放在了大小为512byte的页面里，我们**将存储redo日志的页称为block**
...
实际上在server启动时向系统申请了一大片连续的内存空间，称为"redo log buffer"，简称为**log buffer**
...
MTR运行中产生的一组redo日志**会在MTR结束后会被复制到log buffer中**
...
**当某个页被刷新到了磁盘上(脏页对应数据刷新)，那么该页对应生成的redo日志可以被覆盖了，此时会进行一个增加checkpoint_lsn的操作，该过程被称为执行一次checkpoint**
...
通过变量innodb_flush_log_at_trx_commit**可以控制是否立即将事务对应的所有redo日志刷新到磁盘中，其有三个变量**：
...
之后遍历哈希表，因为对同一个页面修改的redo日志放在了一起，所以可以一次将一个页面修复好(避免了随机I/O)
...
**注意：**如果事务执行到了一半时系统崩了，此时redo日志虽然刷新到了磁盘中，执行一半的事务所做的修改都会被撤销，下一章undo日志中会讲解
