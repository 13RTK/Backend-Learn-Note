...
- 一旦被驱动表的WHERE子句符合空值拒绝的条件，则外连接和内连接可以相互转换
...
2. 与外层查询的关系
    ...
    如果子查询**可以独立运行处结果，不依赖其他外层查询的值**，则这个子查询为不相关子查询
...
// 对于expr IN (arg1, arg2, ...)这种IN子句来说，IN中的参数首先会被排列，如果不能利用索引将IN子句划分为若干扫描区间，则会对已经排序号的参数进行二分查找，加快计算IN的效率
...
- IN只是判断某个操作数是否存在于某个集合，所以集合中的值是否重复并不重要，但去重后可以让临时表变小
